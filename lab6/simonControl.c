...
//how long to display "yay" for until moving on (4 seconds)
#define SUCCESS_WAIT_TIME 40
...
#define GAME_END_LEVEL_COORD_X 280
...
//We can set this to the amuont of cycles it took for the user to start a new game
...
static enum simonControl_st_t currentState = st_init;//always should start with init
...
void simonControl_tick()
{
    ...
    switch(currentState)
    {
        case st_init:
            ...
            {
                if (newSequenceGenerate)//if we need to generate a new seq we are gonna do that then go into a new game
                {
        ...
        case st_wait_for_release:
            //we need to wait for the screen to be release before blanking the screen and drawing the buttons
        ...
        case st_verify_disable:
            ...
            {
                //inc and move back to the start of the flahs and verify sequence
                currentSequencePos++;
                currentState = st_game_start;
            }
            break;
        ...
        case st_success_wait:
            //we need to wait around for a certain amount of time before clearing the message
        ...
        case st_newlevel_wait:
            ...
            else if (newScreenTimer == SUCCESS_WAIT_TIME)// if the user hasnt pressed the screen for 4 seconds they dont want to continue
            {
                //clear the screen
                display_fillScreen(DISPLAY_BLACK);
                currentState = st_end_game_print;
    ...
    switch(currentState)
    {
        case st_init:
            ...
            randSeed++;//we will increment the seed while just waiting here on the title screen
            break;
        case st_wait_for_release:
        ...
        case st_end_game_print:
            ...
            display_println(currentLength - SEQ_START_LEN);
            break;
        case st_end_game_wait:
            endScreenTimer++;
            break;
