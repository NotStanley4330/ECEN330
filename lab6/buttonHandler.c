...
enum buttonHandler_st_t {
  ...
  st_is_touched,  // a state that changes the size of the touched square and
                  // waits
  st_untouched,   // a state for reverting the screen back to nomrla after the
                  // touch is released
...
// gets the touched point on the screen when called then returns a 0, 1, 2, or 3
// from the simonDisplay_computeRegionNumber() function representing the region
// touched
...
void buttonHandler_init() { currentState = st_init; }
...
bool buttonHandler_isComplete() { return buttonHandlerComplete; }
...
void buttonHandler_tick() {
  ...
  case st_untouched:
    ...
    break;
  case st_finished:
    if (!handlerEnable) // wait for the machine to be disabled to go back to the
                        // initial state
    {
      currentState = st_init;
    }
  ...
  case st_await_touch:
    break;
  case st_settle:
    adcSettleTimer++; // incrememnt the sttel timer each tick this state is
                      // still current
    break;
  case st_is_touched:
    break;
