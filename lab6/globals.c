/*
    ...
    Date: 5/31/2022
    Purpose: a file that stores the global sequence and has functions to return
   values when asked
...
void globals_setSequence(const uint8_t sequence[], uint16_t length) {
  ...
  for (int i = 0; i < length; i++) { // copy over the contents of the input
                                     // array
...
uint16_t globals_getSequenceIterationLength() { return currentSeqPos; }